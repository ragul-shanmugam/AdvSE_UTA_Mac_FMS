...
<body><br>
<sql:setDataSource var="dsfacility" driver="com.mysql.jdbc.Driver" url="jdbc:mysql://localhost:3306/mac_fms?autoReconnect=true&useSSL=false" user="root" pa	ssword="MyNewPass"/>
...
<form action="/UTA_Mac_FMS/FacilityController?action=listFacilities" method="POST" name="searchfacility_form">
			<div class="row">
				<div class="col">
					<label for="role">Facility Type</label> <select name="ftype" id="ftype" class="form-control">
					<option value="selectfacility">Select a Facility</option>
						<c:forEach var="row" items="${facilityresult.rows}"> 
  						    <option value='<c:out value="${row.Facility}"/>'><c:out value="${row.Facility}"/></option>
					</c:forEach>
					</select>
				</div>
				<div class="col"></div>
